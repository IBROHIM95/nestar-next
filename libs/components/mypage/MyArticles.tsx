	...
	const likeBoArticleHandler = async (e:any, user:any , id:string) => {
		try{
		 ...
		 if (!user._id) throw new Error(Messages.error2)
		   ...
		   console.log('ERROR, likePropeprtyHandler', err.message);
				...
				<Stack className="article-list-box">
					{boardArticles?.length > 0 ? (
						boardArticles?.map((boardArticle: BoardArticle) => {
							return <CommunityCard likeArticleHandler={likeBoArticleHandler} boardArticle={boardArticle} key={boardArticle?._id} size={'small'} />;
						})
					) : (
